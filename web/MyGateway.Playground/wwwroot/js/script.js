const CLIENT_ID = ""; //Coloque seu CLIENT ID aqui
const CLIENT_SECRET = ""; //Coloque seu CLIENT SECRET aqui
const API_PORT = 7183;
...
async function getAuthToken() {
  ...
  return $.ajax({
    async: true,
    crossDomain: true,
    url: `https://localhost:${API_PORT}/authentication/v1/auth`,
    method: "POST",
    headers: {
...
async function initializeSDK() {
  ...
  return $.ajax({
    async: true,
    crossDomain: true,
    url: `https://localhost:${API_PORT}/antifraud/v1/sdk/3ds/initialize`,
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Accept: "*/*",
...
function validateChallengeCallback(jwt, statusChallenge) {
  ...
  const settings = {
    async: true,
    crossDomain: true,
    url: `https://localhost:${API_PORT}/payments/v1/creditcard/antifraud/validate/3ds`,
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Accept: "*/*",
...
function validateResponse(status) {
  if (status == "success") {
    alert(
      "Desafio concluído, acompanhe o polling da situação do pagamento através do console"
    );
    console.log(
...
async function setCardToken(callback) {
  ...
  $.ajax({
    url: `https://localhost:${API_PORT}/payments/v1/creditcard/generate/token`,
    type: "POST",
    headers: {
      "x-api-key": $('meta[name="_api_key"]').attr("content"),
      Authorization: authToken,
...
async function getPaymentStatus(paymentId) {
  ...
  $.ajax({
    url: `https://localhost:${API_PORT}/payments/v1/situation/${paymentId}`,
    type: "GET",
    async: true,
    headers: {
      "x-api-key": $('meta[name="_api_key"]').attr("content"),
  ...
  form.addEventListener("submit", async function (event) {
    ...
    if (isValid) {
      await creditCardPayment(() => {
        ...
        $.ajax({
          url: `https://localhost:${API_PORT}/payments/v1/create`,
